...
// --- Model --- // 
 ...
 for (n in 1:N) {
   target += log_mix(beta_vec[n],
                     normal_lpdf(y[n] | mu[2, n], sigma[2]) - normal_lccdf(max_mfi | mu[2, n], sigma[2]),
                     normal_lpdf(y[n] | mu[1, n], sigma[1]) - normal_lccdf(max_mfi | mu[1, n], sigma[1]));
 }
...
generated quantities {
  ...
  for (n in 1:N) {
   ...
   log_beta[1, n] += normal_lpdf(y[n] | mu[2, n], sigma[2]) - normal_lcdf(max_mfi | mu[2, n], sigma[2]);
   log_beta[2, n] += normal_lpdf(y[n] | mu[1, n], sigma[1]) - normal_lcdf(max_mfi | mu[1, n], sigma[1]);
